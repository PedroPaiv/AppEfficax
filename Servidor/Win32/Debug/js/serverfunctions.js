...
function TsmGeral(connectionInfo)
{
  this.executor = new ServerFunctionExecutor("TsmGeral",connectionInfo);

  /*
   * @param Sender [in] - Type on server: TObject
   */
  this.FDConnection1BeforeConnect = function(Sender) {
    this.executor.executeMethod('"FDConnection1BeforeConnect"', "POST", [Sender], arguments[1], false, arguments[2], arguments[3]);
  };
  ...
  /*
   * @param vFiltro [in] - Type on server: string
   * @return result - Type on server: TFDJSONDataSets
   */
  this.GetProduto = function(vFiltro) {
    var returnObject = this.executor.executeMethod('GetProduto', "GET", [vFiltro], arguments[1], true, arguments[2], arguments[3]);
    if (arguments[1] == null) {
      if (returnObject != null && returnObject.result != null && isArray(returnObject.result)) {
        var resultArray = returnObject.result;
        var resultObject = new Object();
        resultObject.vFiltro = vFiltro;
        resultObject.result = resultArray[0];
        if (returnObject.cacheId != null && returnObject.cmdIndex != null) {
          resultObject._cacheId = returnObject.cacheId;
          resultObject._cmdIndex = returnObject.cmdIndex;
  ...
  this.GetProduto_URL = function(vFiltro) {
    return this.executor.getMethodURL("GetProduto", "GET", [vFiltro], arguments[1])[0];
  };

  this.PutProduto = function() {
    this.executor.executeMethod('PutProduto', "GET", [], arguments[0], false, arguments[1], arguments[2]);
  };

  this.PutProduto_URL = function() {
    return this.executor.getMethodURL("PutProduto", "GET", [], arguments[0])[0];
  };
}
...
var JSProxyClassList = {
  ...
  "TsmGeral": ["FDConnection1BeforeConnect","EchoString","ReverseString","GetProduto","PutProduto"]
};
